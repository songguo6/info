export const bsn = [{
  name: 'BSN',
  url: 'https://www.bsnbase.com',
  logo: 'https://www.bsnbase.com/img/web_portal/web_portal_icon2.png',
},{
  name: 'BSN Global',
  url: 'https://global.bsnbase.com/',
  logo: 'https://www.bsnbase.com/img/web_portal/web_portal_icon2.png',
},{
  name: 'BSN官方知识库',
  url: 'http://kb.bsnbase.com/home/Pubindex.html',
  logo: 'https://www.bsnbase.com/img/web_portal/web_portal_icon2.png',
},{
  name: 'BSN培训',
  url: 'https://www.bsnbase.com/p/main/BSNActivity?type=Cultivate',
  logo: 'https://www.bsnbase.com/img/web_portal/web_portal_icon2.png',
}]

export const policy = [{
  name: '区块链信息服务备案管理',
  url: 'https://bcbeian.ifcert.cn/index',
  logo: 'https://5b0988e595225.cdn.sohucs.com/images/20190919/fcd006a6afe34d7aa34b8b92149755d5.jpeg',
}]

export const company = [{
  name: '蚂蚁区块链',
  url: 'https://antchain.antgroup.com/',
  logo: 'https://gw.alipayobjects.com/zos/bmw-prod/d7490aea-bbd4-4031-97c4-497ee4d19be3.ico',
},{
  name: '腾讯区块链',
  url: 'https://trustsql.qq.com/',
  logo: 'https://trustsql.qq.com/chain_oss/web/trustsql/res/img/global/favicon.ico',
},{
  name: '百度超级链',
  url: 'https://xuperchain.baidu.com/n/ps/opensource',
  logo: 'https://xuperchain.baidu.com/n/static/platform-home/static/images/title/favicon_0b017ba.png',
},{
  name: '京东智臻链',
  url: 'https://blockchain.jd.com/',
  logo: 'https://blockchain.jd.com/favicon.png',
},{
  name: '华为云区块链',
  url: 'https://www.huaweicloud.com/product/bcs.html',
  logo: 'https://res-static3.huaweicloud.com/content/dam/cloudbu-site/archive/commons/web_resoure/framework/favicon/favicon.ico?sttl=202008280900',
},{
  name: '浪潮云区块链',
  url: 'https://cloud.inspur.com/solution/Blockchain/',
  logo: 'https://xavatar.imedao.com/community/20183/1524446231560.png!180x180.png',
},{
  name: '迅雷链',
  url: 'https://blockchain.xunlei.com/',
  logo: 'https://tvax1.sinaimg.cn/crop.0.0.200.200.180/007duQKqly8ftvhb4vbhwj305k05kglr.jpg?KID=imgbed,tva&Expires=1598635015&ssig=ew2TFkSIoM',
},{
  name: '平安壹账链',
  url: 'https://baas.yizhangtong.com/',
  logo: 'https://baas.yizhangtong.com/logoicon.png',
},{
  name: '360区块链安全',
  url: 'https://bcsec.360.cn/',
  logo: 'https://bcsec.360.cn/assets/imgs/favicon.13901a67ddb198064ee3.ico',
},{
  name: '33复杂美',
  url: 'https://www.33.cn/',
  logo: 'https://chain.33.cn/chain33.ico',
},{
  name: '中链科技',
  url: 'http://www.sinochain.com/',
  logo: 'http://static.websiteonline.cn/website/template/default/images/favicon.ico',
},{
  name: '远光区块链',
  url: 'http://www.ygsoft.com/zt/blockchain/index.html',
  logo: 'http://www.ygsoft.com/zt/blockchain/bitbug_favicon.ico',
},{
  name: '云象区块链',
  url: 'https://www.yunphant.com/',
  logo: 'https://www.yunphant.com/favicon.ico',
},{
  name: '布比区块链',
  url: 'https://bubi.cn/',
  logo: 'https://bubi.cn/bubi.png',
},{
  name: '众享比特',
  url: 'http://www.peersafe.cn/',
  logo: 'http://www.peersafe.cn/favicon.ico',
},{
  name: '欧链科技',
  url: 'https://oulian.io/',
  logo: 'https://tvax3.sinaimg.cn/crop.0.0.100.100.180/006Vzew1ly8fv12v2dxelj302s02smwy.jpg?KID=imgbed,tva&Expires=1598629020&ssig=AQ0s9ooLja',
}]

export const newbase = [{
  name: '人工智能（AI）【数据处理】',
  logo: 'https://cdn.iconscout.com/icon/premium/png-256-thumb/construct-22-123385.png',
},{
  name: '区块链（Blockchain）【数据信任】',
  logo: 'https://cdn.iconscout.com/icon/premium/png-256-thumb/construct-22-123385.png',
},{
  name: '云计算（Cloud Computing）【数据处理】',
  logo: 'https://cdn.iconscout.com/icon/premium/png-256-thumb/construct-22-123385.png',
},{
  name: '大数据（Big Data）',
  logo: 'https://cdn.iconscout.com/icon/premium/png-256-thumb/construct-22-123385.png',
},{
  name: '新能源充电桩（New Energy）',
  logo: 'https://cdn.iconscout.com/icon/premium/png-256-thumb/construct-22-123385.png',
},{
  name: '智能工厂（Smart Factory）',
  logo: 'https://cdn.iconscout.com/icon/premium/png-256-thumb/construct-22-123385.png',
},{
  name: '第5代移动通信技术（5G）【数据传输】',
  logo: 'https://cdn.iconscout.com/icon/premium/png-256-thumb/construct-22-123385.png',
},{
  name: '特高压（UHV）',
  logo: 'https://cdn.iconscout.com/icon/premium/png-256-thumb/construct-22-123385.png',
},{
  name: '工业互联网（Industrial Internet）【数据搜集】',
  logo: 'https://cdn.iconscout.com/icon/premium/png-256-thumb/construct-22-123385.png',
}]

export const infos = [{
  name: '皮书数据库',
  url: 'https://www.pishu.com.cn/',
  logo: 'https://www.pishu.cn/images/32.ico',
},{
  name: '赛迪网',
  url: 'http://www.ccidnet.com/',
  logo: 'http://www.ccidnet.com/favicon.ico',
}]